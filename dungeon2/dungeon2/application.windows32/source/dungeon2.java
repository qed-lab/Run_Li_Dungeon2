...
class Hero{
  ...
  public boolean move(int n){ // 1: left  2: right   3:up   4:down
    int i = -1, j = -1;
    switch(n){
  ...
  public void collideWith(Treasure t){
    score += t.reward;
    y = t.y;
    x = t.x;
    t.getHurt();
  }
  public void collideWith(Potion p){
    HP += p.heal;
    if(HP > 10) HP = 10;
    y = p.y;
    x = p.x;
  ...
  public void collideWith(Goblin g){
    getHurt(g.attack);
    y = g.y;
    x = g.x;
    g.getHurt();
  }
  public void collideWith(Wizard w){
    y = w.y;
    x = w.x;
    w.getHurt();
  }
  public void collideWith(Blob b){
    getHurt(b.attack);
    b.HP -= attack;
    if(b.HP < 1){
      y = b.y;
  ...
  public void collideWith(Ogre o){
    getHurt(o.attack);
    o.HP -= attack;
    if(o.HP < 1){
      y = o.y;
  ...
  public void collideWith(Minotaur m){
    getHurt(m.attack);
    m.getHurt();
  }  
}
...
class Blob{
  ...
  public void display(){
    ...
    image(blobIcon, x*tile + tile*1.5f, y*tile + tile*1.5f);
    imageMode(CORNER);
  }
  public boolean isAt(int y, int x){
    return this.y == y && this.x == x;
...
public boolean noGoblinOrWizard(int y, int x){
  for(Goblin g : goblins){
    if(g.isAt(y, x)){
      return false;
    }
...
public boolean noMinotaur(int y, int x){
  for(Minotaur m : minotaurs){
    if(m.isAt(y, x)){
      return false;
    }
...
public int[] findPath(Minotaur m){
...
public int calcH(int y, int x, int ty, int tx){
